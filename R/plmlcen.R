    ...
    for (ip in c(2, 3)) {
      if (ip == 2) {#& (b2==1|b2==0)
       ...
       #   df[df$id == irow , "y3b2p"] <- mucond
          #######
          if(mucond<min(donor$y3b2p)){
            censor<-max(min(donor$y3b2p),mucond)
          } else if(mucond>max(donor$y3b2p)){
            censor<-min(max(donor$y3b2p),mucond)
          }else{
            censor<-mucond
          }
          #########
          df[df$id == irow , "y3b2p"] <- censor
          ##############
          df$y3b3p <- df$y3b2p
          dft<-df
        }
        ...
        for (b2 in c(1, 0)) {
          if (b2 == 1& (ip==3)) {
            ...
            for (ind in seq(1:nr[1])) {
              ...
              #######
              if(mucond<min(donor$y3b3p)){
                censor<-max(min(donor$y3b3p),mucond)
              } else if(mucond>max(donor$y3b3p)){
                censor<-min(max(donor$y3b3p),mucond)
              }else{
                censor<-mucond
              }
              #########
              df[df$id == irow , "y3b3p"] <- censor
              #df$y3b3p<-df$y3b2p
            }
          }
        }
